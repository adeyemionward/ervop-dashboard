...
export default function EditVendor() {
  ...
  useEffect(() => {
    const fetchVendor = async () => {
      try {
        ...
        const res = await fetch(`${BASE_URL}/professionals/vendors/show/${vendorId}`, {
          headers: {
        ...
        if (result.status && result.vendor) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/professionals/vendors/update/${vendorId}`, {
        method: "PUT",
        headers: {
    ...
    <DashboardLayout>
      <div className="w-full max-w-4xl mx-auto">
        <HeaderTitleCard
          ...
          title="Edit Vendor"
          description="Update vendor details and account information."
        />

        <div className="bg-white p-8 rounded-xl shadow-sm">
          {loading ? (
            <p className="text-gray-500 text-lg">Loading vendor details...</p>
          ) : (
